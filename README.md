...
* 期末自動計算 **2025 台灣常模**（Cheng et al., *J Chin Med Assoc* 2025;88(8):617-623）：校正分數、PR、z（*v3.0 已校正抽象領域年齡係數符號*）
* PDF 報告輸出（含受測者手繪圖，*v3.0 已修復非同步解碼導致列印空白的問題*）
* 雲端自動上傳（*v3.0 新增*）- 進入結果頁時，自動、靜默將個案資料與手繪圖上傳至施測人員設定之 Google Drive，並支援透過 `merge_data.py` 將結構問卷與 MoCA-T 資料雙軌合併輸出至同一個 Excel 報表（獨立分頁）。
